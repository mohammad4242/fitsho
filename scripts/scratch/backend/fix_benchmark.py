...
def patch_analyze_stage4():
    with open('analyze_stage4.py', 'r') as f:
        content = f.read()

    # We need to rewrite analyze_stage4 completely to implement all requirements
...
def format_report(benchmark_json_path: str, output_md_path: str) -> None:
    ...
    assert passes + pass_c + quality_issue + unsat + bugs == total_profiles, f"Category sum ({passes + pass_c + quality_issue + unsat + bugs}) does not equal total profiles ({total_profiles})"
    ...
    assert determinism_runs == total_profiles, f"Determinism runs ({determinism_runs}) != total profiles ({total_profiles})"
    ...
    lines.append(f"- Category sum equals total profiles: {passes + pass_c + quality_issue + unsat + bugs == total_profiles}")
