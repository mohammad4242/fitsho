...
def trace_calls(frame, event, arg):
    if event == "return":
        ret = arg
        if isinstance(ret, str) and ret == "DAY_COUNT_MISMATCH":
            print(f"FOUND EXACT STRING RETURNED in {frame.f_code.co_name} at {frame.f_code.co_filename}:{frame.f_lineno}")
        elif isinstance(ret, tuple) and "DAY_COUNT_MISMATCH" in ret:
            print(f"FOUND TUPLE RETURNED in {frame.f_code.co_name} at {frame.f_code.co_filename}:{frame.f_lineno}")
    return trace_calls
...
for p in benchmark_profiles():
    ...
    if not res.is_success and "DAY_COUNT_MISMATCH" in res.errors:
        print(f"Matched days={req.available_training_days}, level={req.training_experience}")
        break
