...
import app.workouts.program_engine.engine as engine_mod
...
original = engine_mod._day_count_errors
def wrapped(*args, **kwargs):
    ret = original(*args, **kwargs)
    print("WRAPPED:", ret)
    return ret
engine_mod._day_count_errors = wrapped

for p in benchmark_profiles():
    req = profile_to_request(p, enforce_matrix=False)
    if req.available_training_days == 5 and req.training_experience.value == "first_month":
        res = generate_program(req, catalog, RULESET, reference_templates=templates)
        print("FINAL ERRORS:", res.errors)
        break
