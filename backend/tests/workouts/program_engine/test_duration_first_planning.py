...
def test_same_profile_gets_more_useful_capacity_at_sixty_minutes() -> None:
    ...
    assert long_capacity["expected_exercise_count_capacity"] > short_capacity[
        "expected_exercise_count_capacity"
    ]
    assert long_capacity["expected_working_set_capacity"] > short_capacity[
        "expected_working_set_capacity"
    ]
...
def test_short_session_preserves_explicit_priority_above_body_analysis() -> None:
    ...
    assert ranges["chest"]["preferred_weekly_target"] > ranges["glutes"][
        "preferred_weekly_target"
    ]
    assert ranges["chest"]["minimum_direct_sets"] >= RULESET.minimum_coverage_sets[
    ...
    assert ranges["chest"]["actual_direct_volume"] > ranges["glutes"][
        "actual_direct_volume"
    ]
...
def test_duration_planning_never_relaxes_safety_or_equipment() -> None:
    ...
    assert all(
        ExerciseCautionTag.DEEP_KNEE_FLEXION not in item.caution_tags
...
def test_long_session_does_not_exceed_useful_or_hard_volume_to_fill_time() -> None:
    ...
    assert all(
        len(day.exercises) <= RULESET.max_exercises_per_session
        for day in program.weekly_schedule
    )
