...
def make_request(
    ...
    exp: str,
    goal: str,
    priorities: list[MuscleGroup] | None = None
):
    age_map = {
        "beginner": 3,
        "intermediate": 24,
...
def test_set_allocation_invariants(catalog):
    ...
    for days, duration, exp, goal in combinations:
        request = make_request(days, duration, exp, goal, priorities=[MuscleGroup.CHEST, MuscleGroup.BACK])
        result = generate_program(request, catalog, RULESET, reference_templates=())
        assert result.program is not None, f"Failed to generate program for {days}d {duration}m {exp} {goal}: {result.errors}"
        
        for day in result.program.weekly_schedule:
            for ex in day.exercises:
                # We only check sets for resistance exercises. Cardio can be time-based.
                if getattr(ex, "counts_toward_volume", True):
                    assert ex.sets in {3, 4}, f"Exercise {ex.exercise_name} has {ex.sets} sets (must be 3 or 4)"
