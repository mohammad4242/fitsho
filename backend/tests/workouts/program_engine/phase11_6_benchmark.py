"""Independent Phase 11.6 holdout benchmark.
...
PHASE = "11.6"
HOLDOUT_NAMESPACE = "https://fitsho.test/phase11-6-holdout"
...
def _assert_new_population(profiles: Sequence[phase11.BenchmarkProfile]) -> None:
    ...
    if len(profiles) != 150 or len(fingerprints) != 150:
        raise AssertionError("Phase 11.6 must contain 150 unique profiles")
    overlap = fingerprints.intersection(phase11_fingerprints)
    if overlap:
        raise AssertionError(f"Phase 11.6 reuses Phase 11 input fingerprints: {sorted(overlap)}")
    counts = Counter(
...
def _write_summary(payload: Mapping[str, object], output_dir: Path) -> None:
    ...
    lines = [
        "# Phase 11.6 Independent Holdout Benchmark",
        "",
        (
    ...
    (output_dir / "phase11-6-summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run_benchmark(
    ...
    *,
    ...
    (output_dir / "phase11-6-benchmark.json").write_text(
        json.dumps(phase11._jsonable(payload), indent=2, sort_keys=True), encoding="utf-8"
    )
    (output_dir / "phase11-6-profiles.json").write_text(
        json.dumps(
            phase11._jsonable(
                [
                    {
    ...
    with (output_dir / "phase11-6-summary.csv").open("w", newline="", encoding="utf-8") as handle:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the Phase 11.6 independent holdout benchmark")
    parser.add_argument("--database-url", default=None)
    parser.add_argument("--output-dir", type=Path, default=Path("var/benchmarks/phase11-6-holdout"))
