...
@pytest.mark.parametrize(
    ("focus", "muscle", "expected"),
    [
        ("chest_triceps", MuscleGroup.CHEST, FocusAffinity.DEDICATED),
        ("push", MuscleGroup.CHEST, FocusAffinity.GROUPED),
        ("back_biceps", MuscleGroup.BACK, FocusAffinity.DEDICATED),
        ("pull", MuscleGroup.BACK, FocusAffinity.GROUPED),
        ("shoulders_traps", MuscleGroup.SHOULDERS, FocusAffinity.DEDICATED),
        ("push", MuscleGroup.SHOULDERS, FocusAffinity.GROUPED),
        ("back_biceps", MuscleGroup.BICEPS, FocusAffinity.GROUPED),
        ("chest_triceps", MuscleGroup.TRICEPS, FocusAffinity.GROUPED),
        ("biceps", MuscleGroup.BICEPS, FocusAffinity.DEDICATED),
        ("triceps", MuscleGroup.TRICEPS, FocusAffinity.DEDICATED),
        ("upper", MuscleGroup.CHEST, FocusAffinity.NONE),
        ("upper", MuscleGroup.BACK, FocusAffinity.NONE),
        ("upper", MuscleGroup.SHOULDERS, FocusAffinity.NONE),
        ("upper", MuscleGroup.BICEPS, FocusAffinity.NONE),
        ("upper", MuscleGroup.TRICEPS, FocusAffinity.NONE),
        ("full_body", MuscleGroup.CHEST, FocusAffinity.NONE),
    ],
)
def test_focus_affinity_separates_topology_from_user_priority(
    focus: str, muscle: MuscleGroup, expected: FocusAffinity
) -> None:
    policy = PriorityAllocationPolicy.for_request(
        normalize_request(request(priority_muscles=[muscle]), RULESET), RULESET
    )

    assert policy.focus_affinity(focus, muscle) is expected


def test_dedicated_focus_beats_grouped_focus_without_upper_credit() -> None:
    policy = PriorityAllocationPolicy.for_request(
        normalize_request(request(priority_muscles=[MuscleGroup.CHEST]), RULESET), RULESET
    )

    assert policy.focus_affinity("chest_triceps", MuscleGroup.CHEST) > policy.focus_affinity(
        "push", MuscleGroup.CHEST
    )
    assert policy.focus_affinity("upper", MuscleGroup.CHEST) is FocusAffinity.NONE
    assert policy.focus_affinity("full_body", MuscleGroup.CHEST) is FocusAffinity.NONE

    back_policy = PriorityAllocationPolicy.for_request(
        normalize_request(request(priority_muscles=[MuscleGroup.BACK]), RULESET), RULESET
    )
    assert back_policy.focus_affinity("back_biceps", MuscleGroup.BACK) > back_policy.focus_affinity(
        "pull", MuscleGroup.BACK
    )
    assert back_policy.focus_affinity("upper", MuscleGroup.BACK) is FocusAffinity.NONE

    shoulder_policy = PriorityAllocationPolicy.for_request(
        normalize_request(request(priority_muscles=[MuscleGroup.SHOULDERS]), RULESET), RULESET
    )
    assert shoulder_policy.focus_affinity(
        "shoulders_traps", MuscleGroup.SHOULDERS
    ) > shoulder_policy.focus_affinity("push", MuscleGroup.SHOULDERS)
    assert shoulder_policy.focus_affinity("upper", MuscleGroup.SHOULDERS) is FocusAffinity.NONE
...
def test_single_priority_receives_the_capped_emphasis_budget() -> None:
    baseline = _planned_targets(frozenset())
    priorities = frozenset({MuscleGroup.CHEST})
    targets = _planned_targets(priorities)

    assert targets[MuscleGroup.CHEST] - baseline[MuscleGroup.CHEST] == 2
...
def test_priority_program_reports_measurable_emphasis_and_frequency_for_each_priority() -> None:
    ...
    priority = generate_program(
        _six_day_priority_request(priorities=frozenset({MuscleGroup.HAMSTRINGS})),
        full_catalog(),
        RULESET,
    )
    ...
    assert set(volume_trace["priority_muscles"]) == {MuscleGroup.HAMSTRINGS.value}
    ...
    for muscle in (MuscleGroup.HAMSTRINGS,):
...
def test_single_priority_receives_deterministic_emphasis() -> None:
    ...
    first_values = {
        ...
        for muscle in (MuscleGroup.BICEPS,)
    }
    second_values = {
        muscle.value: (
