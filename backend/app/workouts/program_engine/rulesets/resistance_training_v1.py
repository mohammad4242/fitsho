...
class ProgramRuleset:
    ...
    def max_working_sets_for_exercise(
        ...
        *,
        ...
        absolute_cap = min(self.max_working_sets_per_exercise_absolute, 4)
        if strength_set_cap_authorized:
            absolute_cap = min(
                absolute_cap + self.strength_compound_set_cap_bonus,
                5,
            )
        return min(
            cap,
