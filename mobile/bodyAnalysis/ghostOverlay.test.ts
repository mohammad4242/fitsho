...
it("uses the shared front privacy-line golden vector", () => {
  ...
  expect(layout.privacyLine.anchor.y).toBeCloseTo(0.228);
  expect(layout.privacyLine.start.x).toBeCloseTo(0.1);
  expect(layout.privacyLine.end.x).toBeCloseTo(0.9);
  expect(layout.privacyLine.start.y).toBeCloseTo(0.228);
  expect(layout.privacyLine.end.y).toBeCloseTo(0.228);
});
