...
it("rejects unsupported files before making an upload request", () => {
  ...
  expect(screen.getByRole("alert")).toHaveTextContent("فرمت عکس پشتیبانی نمی‌شود");
  expect(api.uploadProfilePhoto).not.toHaveBeenCalled();
});

it("opens a square crop preview before uploading a supported file", () => {
  render(<ProfilePhotoControl initialUrl={null} label="محمد رضایی" />);
  fireEvent.change(screen.getByLabelText("انتخاب عکس پروفایل"), {
    target: { files: [new File(["png"], "avatar.png", { type: "image/png" })] },
  });

  expect(screen.getByRole("dialog", { name: "قاب مربعی عکس" })).toBeInTheDocument();
