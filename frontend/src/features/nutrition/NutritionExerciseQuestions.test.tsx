...
it("collects only minimum structured exercise details for nutrition-only members", async () => {
  ...
  await user.click(screen.getByRole("button", { name: "Mixed training" }));
  await user.click(screen.getByRole("button", { name: "Continue" }));
  await user.click(screen.getByRole("button", { name: "4 days per week" }));
  await user.click(screen.getByRole("button", { name: "Continue" }));
  await user.click(screen.getByRole("button", { name: "45–60 minutes" }));
  await user.click(screen.getByRole("button", { name: "Continue" }));
