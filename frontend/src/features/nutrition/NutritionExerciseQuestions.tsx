...
export function NutritionExerciseQuestions({ initialValue, fitnessGoal, onBack, onComplete }: Props) {
  ...
  function submit(event: FormEvent) {
    ...
    if (question === "trains" && draft.trains === false) {
      if (fitnessGoal === "build_muscle" || fitnessGoal === "muscle_gain") return;
      onComplete({ trains: false });
      return;
    }
    ...
    if (
      draft.exerciseType !== null && draft.days !== null &&
      draft.minutes !== null && draft.intensity !== null
    ) {
      onComplete({
        trains: true,
        exercise_type: draft.exerciseType,
        days_per_week: draft.days,
        minutes_per_session: draft.minutes,
  ...
  const selected = question === "trains" ? draft.trains !== null
    : question === "type" ? draft.exerciseType !== null
    : question === "days" ? draft.days !== null
    : question === "duration" ? draft.minutes !== null
  ...
  const button = (active: boolean, label: string, onClick: () => void) => (
    <button className={active ? "is-selected" : ""} type="button" onClick={onClick}>{label}</button>
  );
    ...
    <section className="guided-question" aria-labelledby="nutrition-exercise-title">
      ...
      <form className="guided-question__form" onSubmit={submit}>
        <div className="guided-choice-grid">
          {question === "trains" && <>
            {button(draft.trains === true, l("منظم تمرین می‌کنم", "I train regularly"), () => setDraft((current) => ({ ...current, trains: true })))}
            {button(draft.trains === false, l("تمرین نمی‌کنم", "I do not train"), () => setDraft({ trains: false, exerciseType: null, days: null, minutes: null, intensity: null }))}
          </>}
          {question === "type" && ([
          ...
          ] as const).map(([value, label]) => button(draft.exerciseType === value, label, () => setDraft((current) => ({ ...current, exerciseType: value }))))}
          {question === "days" && [1, 2, 3, 4, 5, 6, 7].map((value) => button(draft.days === value, l(`${new Intl.NumberFormat("fa-IR").format(value)} روز در هفته`, `${value} days per week`), () => setDraft((current) => ({ ...current, days: value }))))}
          {question === "duration" && durationOptions.map((value, optionIndex) => {
            ...
            return button(draft.minutes === value, labels[optionIndex], () => setDraft((current) => ({ ...current, minutes: value })));
          })}
          {question === "intensity" && ([
            ["light", l("سبک", "Light")],
            ["moderate", l("متوسط", "Moderate")],
            ["vigorous", l("شدید", "Vigorous")],
          ] as const).map(([value, label]) => button(draft.intensity === value, label, () => setDraft((current) => ({ ...current, intensity: value }))))}
        </div>
        {question === "trains" && draft.trains === false && (fitnessGoal === "build_muscle" || fitnessGoal === "muscle_gain") && (
        ...
        <div className="profile-actions">
          <button className="secondary-button" type="button" onClick={back}>{l("بازگشت", "Back")}</button>
          <button className="primary-button" type="submit" disabled={!selected || blockedGoal}>{l("ادامه", "Continue")}</button>
        </div>
      </form>
    </section>
  );
