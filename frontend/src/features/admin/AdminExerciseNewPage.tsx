      ...
      <main className="admin-main admin-main--form">
        ...
        <form className="admin-form" noValidate onSubmit={handleSubmit}>
          ...
          <fieldset className="admin-form-section"><legend>{t("admin.sections.target")}</legend><div className="admin-field-grid">
            <Field label={t("admin.fields.bodyRegion")} error={errorText("body_region")}><select value={form.body_region} disabled={form.needs_review} onChange={(event) => { setField("body_region", event.target.value as AdminExerciseForm["body_region"]); setField("primary_muscle", ""); setField("secondary_muscles", []); }}><option value="">{t("admin.fields.select")}</option>{bodyRegions.map((value) => <option key={value} value={value}>{t(`catalog.bodyRegion.${value}`)}</option>)}</select></Field>
            <Field label={t("admin.fields.primaryMuscle")} error={errorText("primary_muscle")}><select value={form.primary_muscle} disabled={!form.body_region || form.needs_review} onChange={(event) => setField("primary_muscle", event.target.value as MuscleGroup)}><option value="">{t("admin.fields.select")}</option>{availableMuscles.map((value) => <option key={value} value={value}>{t(`catalog.muscle.${value}`)}</option>)}</select></Field>
