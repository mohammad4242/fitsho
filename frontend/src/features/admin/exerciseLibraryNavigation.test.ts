...
it("reads matching upper, lower, and core create contexts", () => {
  expect(readExerciseCreateContext(new URLSearchParams("body_region=upper_body&primary_muscle=shoulders&muscle_focus=rear_delt"))).toEqual({ body_region: "upper_body", primary_muscle: "shoulders", muscle_focus: "rear_delt" });
...
it("rejects mismatched create context and external return paths", () => {
  ...
  expect(exerciseLibraryReturnPath("https://evil.test", "upper_body", "chest", "mid_chest", true, false)).toBe("/exercises?body_region=upper_body&primary_muscle=chest&muscle_focus=mid_chest");
});
...
it("returns review records without a category through the protected review filter", () => {
  expect(exerciseLibraryReturnPath("/exercises?body_region=upper_body", null, null, null, true, true)).toBe("/exercises?admin_status=needs_review");
});
