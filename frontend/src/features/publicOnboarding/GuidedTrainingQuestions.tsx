...
export function GuidedTrainingQuestions({ values, onChange, onBack, onComplete, allowNoTraining = false, onNoTraining }: Props) {
  ...
  const onNoTrainingRef = useRef(onNoTraining);
  useEffect(() => {
    onCompleteRef.current = onComplete;
    onNoTrainingRef.current = onNoTraining;
  });
  ...
  function toggle(caution: TrainingCaution) {
    onChange("training_cautions", cautions.includes(caution) ? cautions.filter((item) => item !== caution) : [...cautions, caution]);
  }
  ...
  const backLabel = language === "en" ? "Back" : "بازگشت";
  const showContinue = question === "trainingAge" || question === "cautions";

  return (
    <section className="guided-question" aria-labelledby="guided-training-title">
      ...
      <form className="guided-question__form" onSubmit={submit}>
        {question === "experience" && (
          <div className="guided-choice-grid">
            {([
              ...
              <button
                className={values.experience_level === value ? "is-selected" : ""}
                key={value}
                type="button"
                onClick={() => selectAndAdvance(
                  () => {
                    setNoTrainingSelected(false);
            ...
            {allowNoTraining && (
              <button
                className={noTrainingSelected ? "is-selected" : ""}
                type="button"
                onClick={() => selectAndAdvance(
                  () => {
                    setNoTrainingSelected(true);
                    onChange("experience_level", "");
                  },
                  () => onNoTrainingRef.current?.(),
                )}
              >
                {language === "en" ? "I do not train" : "تمرین نمی‌کنم"}
              </button>
            )}
        ...
        {question === "trainingAge" && (
          <div className="guided-number-field">
        ...
        {question === "days" && (
          <div className="guided-choice-grid">
            {[2, 3, 4, 5, 6].map((value) => (
              <button
                className={values.training_days_per_week === String(value) ? "is-selected" : ""}
                key={value}
                type="button"
                onClick={() => selectAndAdvance(
              ...
              >
                {language === "en" ? `${value} days per week` : `${new Intl.NumberFormat("fa-IR").format(value)} روز در هفته`}
              </button>
            ))}
          </div>
        )}
        {question === "location" && (
          <div className="guided-choice-grid">
            {["home", "gym"].map((value) => (
              <button
                className={values.training_location === value ? "is-selected" : ""}
                key={value}
                type="button"
                onClick={() => selectAndAdvance(
                  () => onChange("training_location", value),
                  () => setIndex((current) => current + 1),
                )}
              >
                {t(`onboarding.options.trainingLocation.${value}`)}
              </button>
            ))}
          </div>
        )}
        {question === "home" && (
          <div className="guided-choice-grid">
            {["bodyweight_only", "dumbbells_available"].map((value) => (
              ...
              >
                {t(`onboarding.options.homeTrainingSetup.${value}`)}
              </button>
            ))}
          </div>
        )}
          ...
          <div className="guided-choice-grid">
            {([
              ...
              <button
                ...
                type="button"
                onClick={() => selectAndAdvance(
                  () => onChange("session_duration_minutes", String(value)),
                  () => setIndex((current) => current + 1),
                )}
              >
                {label}
        ...
        {question === "intensity" && (
          <div className="guided-choice-grid">
            {([
              ...
              <button
                className={values.training_intensity === value ? "is-selected" : ""}
                key={value}
                type="button"
                onClick={() => selectAndAdvance(
        ...
        {question === "priority" && (
          <div className="guided-choice-grid">
            <button
              className={values.priority_muscle === "" ? "is-selected" : ""}
              type="button"
              onClick={() => selectAndAdvance(
            ...
            {userSelectablePriorityMuscles.map((muscle: UserSelectablePriorityMuscle) => (
              <button
                className={values.priority_muscle === muscle ? "is-selected" : ""}
                key={muscle}
                type="button"
                onClick={() => selectAndAdvance(
        ...
        {question === "cautions" && (
          <div className="guided-choice-grid">
            {(["lower_back", "knee", "shoulder", "neck", "wrist", "other"] as TrainingCaution[]).map((value) => (
              <button
                className={cautions.includes(value) ? "is-selected" : ""}
                key={value}
                type="button"
                onClick={() => toggle(value)}
              >
                {t(`onboarding.options.trainingCaution.${value}`)}
              </button>
            ))}
          </div>
        )}
        {question === "weeks" && (
          <div className="guided-choice-grid">
            {[4, 6, 8].map((value) => (
              <button
                className={values.plan_duration_weeks === String(value) ? "is-selected" : ""}
                key={value}
                type="button"
                onClick={() => selectAndAdvance(
              ...
              >
                {t(`onboarding.options.planDuration.${value}`)}
              </button>
            ))}
          </div>
        )}
        ...
        {showContinue && (
          <div className="profile-actions">
            <button className="primary-button" type="submit" disabled={!ready}>{language === "en" ? "Continue" : "ادامه"}</button>
          </div>
        )}
      </form>
    </section>
