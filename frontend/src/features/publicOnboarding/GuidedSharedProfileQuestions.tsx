    ...
    <section className="guided-question" aria-labelledby="guided-question-title">
      ...
      <form className="guided-question__form" onSubmit={submit}>
        ...
        {question === 1 && <fieldset className="birth-date-picker"><legend>{t("onboarding.fields.birthDate")}</legend>
          <label>{language === "en" ? "Day" : "روز"}<select required value={birthParts.day} onChange={(event) => setBirthParts((current) => ({ ...current, day: event.target.value }))}><option value="" />{Array.from({ length: daysInSelectedMonth }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label>
          <label>{language === "en" ? "Month" : "ماه"}<select required value={birthParts.month} onChange={(event) => setBirthParts((current) => ({ ...current, month: event.target.value }))}><option value="" />{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></label>
          <label>{language === "en" ? "Year" : "سال"}<select required value={birthParts.year} onChange={(event) => setBirthParts((current) => ({ ...current, year: event.target.value }))}><option value="" />{years.map((year) => <option key={year} value={year}>{year}</option>)}</select></label>
        </fieldset>}
