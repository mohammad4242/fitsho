...
import type { GhostViewGeometry, GhostZone } from "./body-ghost.js";
import type { BodyPhotoSide, BodyPhotoView } from "./body-photos.js";
...
export function validatePoseWithGhost(
  ...
  const minVisThreshold = view === "side" ? 0.15 : MIN_LANDMARK_VISIBILITY;

  if (view !== "side" && maxShoulderVis < minVisThreshold) {
    return failResult("shoulders_not_visible", ["body_out_of_frame"], geometry, primaryPose);
  } else if (view === "side" && maxShoulderVis < 0.08 && maxHipVis < 0.12) {
    return failResult("shoulders_not_visible", ["body_out_of_frame"], geometry, primaryPose);
  } else if (view === "side" && maxShoulderVis < 0.15) {
    warnings.push("body_out_of_frame");
  }

  if (maxHipVis < (view === "side" ? 0.12 : minVisThreshold)) {
    return failResult("torso_not_visible", ["body_out_of_frame"], geometry, primaryPose);
  }

  if (
    maxKneeVis < (view === "side" ? 0.12 : minVisThreshold)
    || maxFootVis < (view === "side" ? 0.12 : minVisThreshold)
  ) {
    return failResult("legs_or_feet_not_visible", ["body_out_of_frame"], geometry, primaryPose);
  }

  const shoulderValidThreshold = view === "side" ? 0.15 : minVisThreshold;
  const validShoulderY = [
    primaryPose[LANDMARK_INDICES.leftShoulder],
    primaryPose[LANDMARK_INDICES.rightShoulder],
  ]
    .filter((p): p is NormalizedBodyLandmark => p !== undefined && p.visibility >= shoulderValidThreshold)
    .map((p) => p.y);
  const validHipY = [primaryPose[LANDMARK_INDICES.leftHip], primaryPose[LANDMARK_INDICES.rightHip]]
    .filter((p): p is NormalizedBodyLandmark => p !== undefined && p.visibility >= (view === "side" ? 0.12 : minVisThreshold))
    .map((p) => p.y);
  const validAnkleY = [primaryPose[LANDMARK_INDICES.leftAnkle], primaryPose[LANDMARK_INDICES.rightAnkle]]
    .filter((p): p is NormalizedBodyLandmark => p !== undefined && p.visibility >= (view === "side" ? 0.12 : minVisThreshold))
    .map((p) => p.y);
  ...
  for (const idx of relevantIndices) {
    ...
    if (!pt || pt.visibility < (view === "side" ? 0.12 : MIN_LANDMARK_VISIBILITY)) continue;
    ...
    const overflowLimit = view === "side" ? 0.08 : (isCoreLandmark ? 0.015 : 0.035);
  ...
  if (hasNearBoundary) warnings.push("near_boundary_landmarks");
  ...
  if (bodySpan > 0.95) warnings.push("too_close");
  else if (bodySpan < geometry.expectedBodySpan.min * 0.75) warnings.push("too_far");
  ...
  const shoulderSpan = bothShouldersVisible
    ? Math.abs(
      (primaryPose[LANDMARK_INDICES.leftShoulder]?.x ?? 0)
      - (primaryPose[LANDMARK_INDICES.rightShoulder]?.x ?? 0),
    )
    : 0.04;
  const hipSpan = bothHipsVisible
    ? Math.abs(
      (primaryPose[LANDMARK_INDICES.leftHip]?.x ?? 0)
      - (primaryPose[LANDMARK_INDICES.rightHip]?.x ?? 0),
    )
    : 0.04;
  ...
  function evaluateZone(pt: NormalizedBodyLandmark | undefined, zone: GhostZone) {
    ...
    zoneScoreSum += dist <= 0 ? 1.0 : Math.max(0, 1.0 - dist * 4);
  }
  ...
  if (
    (view !== "side" && minShoulderVis < MIN_LANDMARK_VISIBILITY)
    || (view === "side" && maxShoulderVis < 0.12)
    || (view !== "side" && minHipVis < MIN_LANDMARK_VISIBILITY)
    || (view === "side" && maxHipVis < 0.12)
...
function selectPrimaryPose(poses: NormalizedBodyLandmark[][]): NormalizedBodyLandmark[] | null {
  const valid = poses.filter((pose) => pose.length >= 33);
  return valid[0] ?? null;
}
...
function hasSecondaryCrediblePerson(
  ...
  for (let i = 1; i < poses.length; i++) {
    ...
    const separation = Math.abs(poseCenterX(candidate) - primaryCenter);
    const visCount = candidate.filter((pt) => pt.visibility >= MIN_LANDMARK_VISIBILITY).length;
    if (separation > 0.20 && visCount >= 10) return true;
  }
  return false;
}
...
function determineVisibleLandmarks(
  ...
  const threshold = view === "side" ? 0.10 : MIN_LANDMARK_VISIBILITY;
  ...
  if (vis([LANDMARK_INDICES.leftShoulder, LANDMARK_INDICES.rightShoulder]) >= threshold) list.push("shoulders");
  if (vis([
    LANDMARK_INDICES.leftElbow,
    LANDMARK_INDICES.rightElbow,
    LANDMARK_INDICES.leftWrist,
    LANDMARK_INDICES.rightWrist,
  ]) >= threshold) list.push("arms");
  if (vis([LANDMARK_INDICES.leftHip, LANDMARK_INDICES.rightHip]) >= threshold) list.push("hips");
  if (vis([LANDMARK_INDICES.leftKnee, LANDMARK_INDICES.rightKnee]) >= threshold) list.push("knees");
  if (vis([LANDMARK_INDICES.leftAnkle, LANDMARK_INDICES.rightAnkle]) >= threshold) list.push("ankles");
  if (vis([LANDMARK_INDICES.leftFoot, LANDMARK_INDICES.rightFoot]) >= threshold) list.push("feet");
  return list;
}
...
function calculateMinimumVisibility(
  ...
  if (view === "side") {
    ...
    return Number(Math.min(...sideScores).toFixed(4));
  }
