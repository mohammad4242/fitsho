...
function validateMeasurementFields(
  ...
  for (const field of [
    ...
    if (value === "") {
      if (requireCircumferences) errors[field] = "required";
      continue;
    }
...
function validateStepTwo(values: ProfileFormValues): ProfileValidationErrors {
  ...
  if (values.fitness_goal === "") errors.fitness_goal = "required";
  return errors;
}
...
function validateStepThree(values: ProfileFormValues): ProfileValidationErrors {
  const errors: ProfileValidationErrors = {};
  if (values.experience_level === "") errors.experience_level = "required";
  ...
  } else if (!sessionDurations.some((duration) => duration === Number(sessionDuration))) {
    errors.session_duration_minutes = "sessionDurationInvalid";
  }

  if (values.training_cautions === null) errors.training_cautions = "required";
  if (values.training_intensity === "") errors.training_intensity = "required";
...
export function validateStep(
  ...
  if (step === 1) return validateStepOne(values, today);
  if (step === 2) return validateStepTwo(values);
...
export function toProfilePatch(
  ...
  if (input.display_name !== currentProfile.display_name) patch.display_name = input.display_name;
  if (input.birth_date !== currentProfile.birth_date) patch.birth_date = input.birth_date;
  if (input.sex !== currentProfile.sex) patch.sex = input.sex;
  if (input.height_cm !== currentProfile.height_cm) patch.height_cm = input.height_cm;
  ...
  for (const field of [
    ...
    if (input[field] !== currentProfile[field]) patch[field] = input[field];
  }
  if (input.fitness_goal !== currentProfile.fitness_goal) patch.fitness_goal = input.fitness_goal;
